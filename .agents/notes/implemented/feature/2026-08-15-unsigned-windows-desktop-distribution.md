...
The shared packaging module validates the native Node version, asks pnpm for a hoisted production deployment so package imports do not depend on virtual-store links, runs the deployed CLI, materializes the two workspace override packages inside the bundle, removes unsupported Landlock links, and rejects every remaining dependency link that escapes the runtime. Electron Builder creates both the unpacked application used for verification and the NSIS installer. Neither the application executable nor the installer carries an Authenticode signature.
