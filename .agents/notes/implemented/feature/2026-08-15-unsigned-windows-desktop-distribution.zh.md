...
共享打包模块会校验原生 Node 版本、运行已部署 CLI、在 bundle 内部实体化两个 workspace override 包、移除不支持的 Landlock 链接，并拒绝所有仍然逃出运行时的依赖链接。Electron Builder 同时创建用于验证的解包应用和 NSIS 安装包。应用可执行文件和安装包都不包含 Authenticode 签名。
