...
**Paid:** the image supports only macOS arm64, is large, has no Developer ID signature or notarization, and may require an explicit Gatekeeper override for a quarantined download. It has no auto-update, crash-reporting pipeline, Intel build, or Windows installer. Adding a signed release channel later requires a separate publishing decision and does not change the preview's loopback Web carrier.
