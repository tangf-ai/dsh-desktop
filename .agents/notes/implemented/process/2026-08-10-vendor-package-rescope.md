...
The rewrite touches only **delimited, complete package-name tokens**: quoted or backticked specifiers (optionally with a `/subpath`), `package.json` names and dependency keys, `cordis.yml` `name:` values, and `tsconfig.base.json` `paths` keys. Identically spelled strings that are not package names stay as they are: the `cordis.yml` config-file family; the Loader's literal `cordis:` builtin prefix (`cordis:include`, `cordis:group` — see `vendor/loader/src/config/tree.ts`); the `cordis/*` event family; the `cordis` UI locale namespace, input-trigger name, and event-catalog key; kind strings like `cordis-config-entry`; `@deepseek-ai/dsh-tool-cordis`; vendor directory components such as `cosmokit`; Schemastery's upstream `Symbol.for('schemastery')` and `vendor:` metadata field; the `packages/<group>/` directory names in `GROUP_ORDER` (`scripts/gen-module-graph.ts`, `scripts/gen-doc-graphs.ts`); and the upstream install instructions in `vendor/*/README.md`. The codemod records protocol tokens and per-file product or directory ids explicitly, and postconditions keep representative exclusions executable.
