...
- **Runtime and product identifiers.** The `cordis/*` event family, the `cordis` UI locale namespace and input-trigger name, vendor directory components such as `cosmokit`, and Schemastery's `Symbol.for('schemastery')` and `vendor:` metadata keep their established spelling.
